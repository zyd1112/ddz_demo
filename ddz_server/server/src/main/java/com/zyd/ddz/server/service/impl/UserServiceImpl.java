...
public class UserServiceImpl implements UserService {
    ...
    public UserDto getById(Session session, long uid) {
        return DtoUtils.packUser(CacheManager.getById(UserDomain.class, uid));
    }

    @Override
    public List<UserDto> getList(Session session) {
        return CacheManager.getAll(UserDomain.class).stream().map(DtoUtils::packUser).collect(Collectors.toList());
    }
}
